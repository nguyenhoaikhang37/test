...
import { useCreateBoardMutation } from '../../graphql/gen-types'
      ...
      return Array.from(
        Object.values(state.workspace.blocks)?.filter(
          e =>
            e.__typename === 'Board' &&
            (teamId === e.teamId ||
              (teamId === WorkspaceConfig.personalTeam && !e.teamId))
        )
      )
    }) || []
        ...
        <IconButton
          ...
          onClick={() => {
            createBoard({
              variables: {
                boardInput: {
                  title: 'new board' + teamId,
                  teamId:
                    teamId === WorkspaceConfig.personalTeam ? undefined : teamId
                }
              }
            })
          }}
